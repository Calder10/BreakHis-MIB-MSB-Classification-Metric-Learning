...
def create_fold(f,m):
    ...
    for row in db.readlines():
        columns = row.split('|')
        imgname = columns[0]
        mag = int(columns[1])  # 40, 100, 200, or 400
    ...
    path = "../embeddings/%dX/test/test_images_path_f%d" %(m,f)
...
def train_net(triplet_net,x_train,y_train,f,m):
    ...
    path = "../models/%dX/triplet_net_%d.h5" %(m,f)
    triplet_net.save(path)
    print("Model saved !")
    #plot_loss(history)
    return triplet_net

def save_embedding(triplet_net,f,x_train,y_train,x_test,y_test,m):
    ...
    train_emb = "../embeddings/%dX/train/train_emb_f%d.npz" %(m,f)
    test_emb = "../embeddings/%dX/test/test_emb_f%d.npz" %(m,f)
...
def run_5_folds():
    ...
    for i in range(1,6):
        for m in mf:
            print("Fold %d Magnification factor %dX" %(i,m))
            x_train,y_train,x_test,y_test = create_fold(i,m)
            triplet_net = create_triplet_net()
            triplet_net = train_net (triplet_net,x_train,y_train,i,m)
            save_embedding(triplet_net,i,x_train,y_train,x_test,y_test,m)
