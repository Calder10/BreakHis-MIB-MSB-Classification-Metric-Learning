...
def upload_train_test_set():
    global train_path, test_path, train_dataset, test_dataset, train_loader, test_loader, val_path,val_dataset,val_loader
...
def create_folder_fold():
    ...
    for i in ingr:
        os.mkdir("../fold/"+i)
        os.mkdir("../fold/"+i+"/train")
        os.mkdir("../fold/"+i+"/test")
        os.mkdir("../fold/"+i+"/val")
        for c in classes:
            os.mkdir("../fold/"+i+"/train/"+c)
            os.mkdir("../fold/"+i+"/test/"+c)
            os.mkdir("../fold/"+i+"/val/"+c)
...
def create_fold(f):
    ...
    for row in db.readlines():
        ...
        if pi in val_patients[f-1]:
            dest_path = "../fold/"+mag+"X"+"/val/"+label
            copy2(srcfile,dest_path)
        else:
            if grp == 'train':
                dest_path = "../fold/"+mag+"X"+"/train/"+label
                copy2(srcfile,dest_path)
            else:
                dest_path = "../fold/"+mag+"X"+"/test/"+label
                copy2(srcfile,dest_path)
    print("Done !")
...
def create_embedding():
    global net,train_loader ,test_loader, device,train_emb, test_emb,val_emb
    x_train = []
    y_train = []
    y_test = []
    x_test = []
...
def create_embeddings(f):
    global train_path, test_path, train_emb, test_emb,test_images_path_p, val_path , val_emb
    ingr = ['40X', '100X', '200X', '400X']
    for i in ingr:
        print("Magnification factor",i)
        train_path = "../fold/"+str(i)+"/train"
        test_path = "../fold/"+str(i)+"/test"
        val_path = "../fold/"+str(i)+"/val"
        train_emb = "../embeddings/"+str(i)+"/train/train_emb_f"+str(f)+".npz"
        test_emb = "../embeddings/"+str(i)+"/test/test_emb_f"+str(f)+".npz"
        val_emb = "../embeddings/"+str(i)+"/test/val_emb_f"+str(f)+".npz"
        test_images_path_p = "../embeddings/"+str(i)+"/test/test_images_path_f"+str(f)
